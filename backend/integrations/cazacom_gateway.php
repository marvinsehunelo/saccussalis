...
// Usage: require_once __DIR__ . '/config/integration.php'; and call sendSmsToCazaCom($to, $message, $sender)
 ...
 */
function sendSmsToCazaCom(string $to, string $message, string $sender = null, int $attempt = 0): array {
    global $pdo, $config;

    $url = $config['CAZACOM_SMS_URL'];
    $apiKey = $config['CAZACOM_API_KEY'];
    ...
    // Log attempt to sms table (columns: user_id, sender_number, target_number, message, cost, status, provider_response, created_at)
    try {
        $stmt = $pdo->prepare("
            INSERT INTO sms (user_id, sender_number, target_number, message, cost, status, provider_response, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, NOW())
        ");
        // user_id NULL (system). Adjust if you know user_id.
    ...
    if ($attempt < 3) {
        sleep(pow(2, $attempt));
        return sendSmsToCazaCom($to, $message, $sender, $attempt + 1);
    }
